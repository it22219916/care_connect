...
function PrescriptionForm(props) {
  ...
  const getMedicines = async () => {
    const response = await axios.get(Config.get("getMedicines"));
  ...
  return (
    <form name={props.formName} onSubmit={props.formOnSubmit}>
      ...
      <div className="medicineDiv">{medicines && renderMedicineItems()}</div>
      <div className="text-center">
        <Button
          variant="outlined"
          onClick={addMedicineItem}
          className="my-3 mx-4"
          style={{ border: "1px solid green", color: "darkgreen" }}
          startIcon={<AddIcon />}
        >
          Add Medicine
        </Button>
      ...
      <div>
        <input
          ...
          name="prescribedMed"
          id="prescribedMed"
          value={JSON.stringify(medicineItems)}
        />
      </div>
      <div className="text-center">
        <input
