...
export default function DoctorAppointmentTable({
  ...
  const [openPrescriptionFormDialogue, setOpenPrescriptionFormDialogue] =
    React.useState(false);
  ...
  const prescriptionFormSubmitted = async (event) => {
    ...
    console.log(reqObj);
    let response = await axios.post(Config.get("getPrescriptions"), reqObj, {
      headers: {
                        ...
                        return (
                          <TableCell key={column.id} align={column.align}>
                            {column.format && typeof value === "number" ? (
                              ...
                              <NavLink
                                to={`/patient/history/${row["patientID"]}`}
